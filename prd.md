...
功能界面：需提供每个功能的页面菜单，使用平铺的菜单风格， 不要使用一二级菜单风格，而是每个功能点都有一个独立的页面菜单，点击后跳转到对应的功能页面。不使用dark风格。
...
1. data 目录:存放 rag 需要的文档，用于构建向量知识库
...
1. 对于每个需求必须完全实现，并能给出明确的测试步骤，以证明符合验收要求

## 4.专家智能体优化版评分标准(总分 100 分) 

4.1.基础数据层建设(25 分)

##### 4.1.1.菜单：PDF 知识库构建 (5 分)
...
C、数据分布：实现基于性别(男女)、年龄段、身高、 体重维度的可视化图表展示，截图展示(3 分)



#### 4.1.4. 菜单：多模态数据整合 (7 分)

- 功能点：智能对话，实现 PDF+Excel+MySQL 的统一检索接口，查询结果需要展示"高血压急症处理"的跨源检索结果，包含 PDF 指南、Excel 数据、MySQL 数据库中的相关信息(4 分)
...
#### 4.2.1. 菜单：临床数据查询与分析（临床数据查询与分析，以智能对话方式完成下述功能） (15 分)

- 患者画像构建，输入患者ID(字符串)，智能体自动查询 MySQL数据 生成完整患者画像(年龄、 性别、病史、用药、检查结果)(5 分)

- 风险分层评估，基于查询结果，智能体再次调用MySQL查询高血压风险评估表及rag知识库，生成风险等级和随访计划，并展示评估逻辑(5 分)

- 用药冲突检测: 智能体检测患者当前用药是否存在禁忌或相互作用(rag知识库)，展示预警提示 (5 分)

#### 4.2.2. 菜单：个性化诊疗决策（个性化诊疗决策）(20 分)
